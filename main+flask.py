...
# parser.add_argument('--sentiment', type=str, default='0', help='sentiment for system. 0 is neutral')
parser.add_argument('--model_params', type=str, default='orangutan.ckpt')
...
class KoGPT2Finetuning(LightningModule):
    ...
    def filterchatting(self, user_chat, sent='0'):
        tok = TOKENIZER
        ...
        with torch.no_grad():
            ...
            f = ''
            while 1:
                input_ids = torch.LongTensor(tok.encode(U_TKN + o + SENT + sent + S_TKN + f)).unsqueeze(dim=0)
                pred = self(input_ids)
...
if __name__ == "__main__":
    if args.train:
        ...
        # !CUDA_VISIBLE_DEVICES=0 python main.py --train --gpus 1 --max_epochs 10
    ...
    if args.chat:
        ...
        def hello_user(chats):
            return model.filterchatting(chats)
